...
s=importlib.util.spec_from_file_location('builder',BASE/'cc2_firmware_builder_v3.7.py');b=importlib.util.module_from_spec(s);s.loader.exec_module(b)
def reject(fn):
 try:fn()
...
def main():
 ...
 r=subprocess.run(['python3',str(BASE/'cc2_firmware_builder_v3.7.py'),'--signing-mode','stock','--check-signing-key-only'],capture_output=True,text=True)
