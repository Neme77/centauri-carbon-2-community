#!/usr/bin/env python3
"""Centauri Carbon 2 02.01.00.00 unified Dual-Trust firmware builder V3.7.

Builds the following chain (v3.5 is not hardware-validated):
 official stock .zip.sig -> verify/decode stock SWU -> Z-offset + OpenSSH + Dual Trust v2 + HTTP/upload v1 -> rebuilt SWU
...
V3.7 uses ZIP_STORED and Dual Trust v2. Historical v1 golden image hashes
...
def verify_signing_key(private,mode='community'):
 ...
 challenge=hashlib.sha256(b'CC2 builder v3.7 local signing-key check'+os.urandom(32)).digest()
...
def main():
 ap=argparse.ArgumentParser(description='CC2 02.01.00.00 Dual-Trust firmware builder V3.7 self-contained + functional golden regression checks')
 here=Path(__file__).resolve().parent
 ...
 a.output=a.output or ('CC2_FULL_V3_7_'+release_tag+'.zip.sig')
 ...
 try:
  ...
  reqhash(printer,PRINTER_PATCHED_SHA256,'installed HTTP/upload v1 elegoo_printer')
  ...
  reqhash(vr/'opt/bin/elegoo_printer',PRINTER_PATCHED_SHA256,'rebuilt HTTP/upload v1 elegoo_printer')
  ...
  (cpio/'cpio_item_md5').write_bytes(data); swu=work/'CC2_02.01.00.00_SSH_ZOFFSET_DT2_HTTP_UPLOAD1.swu'
  ...
  swusig_name='CC2_02.01.00.00_SSH_ZOFFSET_DT2_HTTP_UPLOAD1_'+release_tag+'.swu.sig'; swusig=work/swusig_name
  ...
  z=work/('CC2_FULL_V3_7_'+release_tag+'.zip')
  ...
  if a.require_golden:
   # V3.7: byte hashes of rebuilt SquashFS/SWU/ELEG are not deterministic because
  ...
  print('elegoo_printer MD5:',PRINTER_PATCHED_MD5)
