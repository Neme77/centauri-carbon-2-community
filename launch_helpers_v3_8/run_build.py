#!/usr/bin/env python3
"""Run the existing v3.8 builder with fixed local inputs and separate logs/outputs."""
import argparse
import datetime
import hashlib
...
BUILDER_SHA256 = '4cbdb07214952499067c89fceff70be6cc3399abb433191d4e5a722252f448ef'
...
def build_arguments(root, mode, preflight=False, check_key=False, keep_work=False):
    ...
    builder = root / 'cc2_builder_v3_8/cc2_firmware_builder_v3.8.py'
    ...
    stem = 'CC2_V3_8_' + mode.upper() + '_' + stamp
    if check_key:
    ...
    if sha256(builder) != BUILDER_SHA256:
        raise RuntimeError('Builder diverso dalla v3.8 prevista. Non avvio la costruzione.')
...
def run(root, mode, preflight=False, check_key=False, keep_work=False):
    ...
    with log.open('x', encoding='utf-8') as dst:
        dst.write('CC2 v3.8 launcher; signing mode=' + mode + '\n')
        dst.flush()
